...
export async function getMacroSnapshot(): Promise<MacroSnapshot> {
  ...
  console.log(`[MACRO] CPI ${merged.cpi_yoy}% | Core ${merged.core_cpi_yoy}% | Fed ${merged.fed_rate}% | M2 ${merged.m2_growth}% | ETF ${merged.etf_flow_7d}M (asOf ${merged.etf_flow_asof ?? '?'}) | F&G ${merged.fear_greed} | BTC.D ${merged.btc_dominance}% | DXY ${merged.dxy} | Gold $${merged.gold_price} | source: ${merged.source_note} | fgRes:${fgRes.status} cgRes:${cgRes.status}`)
